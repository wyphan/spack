...
def build_criteria_names(costs, tuples):
    ...
    criteria = [(p - fixed_priority_offset + num_build, None, name) for p, name in fixed]
...
class SpackSolverSetup(object):
    ...
    def preferred_targets(self, pkg_name):
        ...
        self.gen.fact(fn.package_target_weight(str(preferred.architecture.target), pkg_name, -30))
    ...
    def build_version_dict(self, possible_pkgs, specs):
        ...
        for pkg_name in possible_pkgs:
            ...
            for idx, item in enumerate(sorted(pkg.versions.items(), key=key_fn, reverse=True)):
                v, version_info = item
    ...
    def define_target_constraints(self):
        ...
        for spec_name, target_constraint in sorted(self.target_constraints):
            ...
            for target in allowed_targets:
                self.gen.fact(fn.node_target_satisfies(spec_name, target_constraint, target))
            self.gen.newline()
    ...
    def setup(self, driver, specs, tests=False, reuse=False):
        ...
        for spec in specs:
            missing_deps = [d for d in spec.traverse() if d.name not in possible and not d.virtual]
            if missing_deps:
        ...
        for spec in sorted(specs):
            ...
            self.gen.fact(fn.virtual_root(spec.name) if spec.virtual else fn.root(spec.name))
...
#
def solve(specs, dump=(), models=0, timers=False, stats=False, tests=False, reuse=False):
