...
def generate_package_index(cache_prefix):
    ...
    sorted_specs = sorted(all_mirror_specs.keys(), key=lambda k: all_mirror_specs[k]["num_deps"])
            ...
            if num_deps > 0:
                ...
                if to_splice:
                    ...
                    for true_dep in to_splice:
                        tty.debug("      {0}/{1}".format(true_dep.name, true_dep.dag_hash()[:7]))
                    ...
                    web_util.push_to_url(temp_json_path, spliced_spec_url, keep_original=False)
...
def download_tarball(spec, preferred_mirrors=None):
    ...
    if preferred_mirrors:
        for preferred_url in preferred_mirrors:
            urls_to_try.append(url_util.join(preferred_url, _build_cache_relative_path, tarball))
...
def get_mirrors_for_spec(
    ...
    if not results and not index_only:
        results = try_direct_fetch(spec, full_hash_match=full_hash_match, mirrors=mirrors_to_check)
...
def needs_rebuild(spec, mirror_url, rebuild_on_errors=False):
    ...
    try:
        ...
        try:
            ...
            tty.error(
                "".join(err_msg).format(spec.short_spec, specfile_path, deprecated_specfile_path)
            )
            tty.debug(url_err)
            tty.debug(url_err_y)
    ...
    spec_dict = (
        sjson.load(spec_file_contents) if specfile_is_json else syaml.load(spec_file_contents)
    )
    ...
    if nodes and "name" not in nodes[0]:
        ...
        cached_pkg_specs = [item for item in nodes if item[ht.dag_hash.name] == spec.dag_hash()]
    ...
    if not cached_target:
        ...
        if full_hash != pkg_full_hash:
            reason = "hash mismatch, remote = {0}, local = {1}".format(full_hash, pkg_full_hash)
            rebuild = True
...
def download_single_spec(concrete_spec, destination, require_cdashid=False, mirror_url=None):
