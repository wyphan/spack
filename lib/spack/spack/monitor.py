...
def get_client(host, prefix="ms1", allow_fail=False, tags=None, save_local=False):
    ...
    cli = SpackMonitorClient(
        host=host, prefix=prefix, allow_fail=allow_fail, tags=tags, save_local=save_local
    )
    ...
    if not save_local:
        ...
        if info:
            tty.debug("%s v.%s has status %s" % (info["id"], info["version"], info["status"]))
    return cli
...
class SpackMonitorClient:
    ...
    def __init__(self, host=None, prefix="ms1", allow_fail=False, tags=None, save_local=False):
    ...
    def authenticate_request(self, originalResponse):
        ...
        headers.update(
            {"service": h.Service, "Accept": "application/json", "User-Agent": "spackmoncli"}
        )
    ...
    def send_analyze_metadata(self, pkg, metadata):
        ...
        data = {"build_id": self.get_build_id(pkg.spec, spec_exists=False), "metadata": metadata}
    ...
    def send_phase(self, pkg, phase_name, phase_output_file, status):
        ...
        data.update(
            {"status": status, "output": read_file(phase_output_file), "phase_name": phase_name}
        )
