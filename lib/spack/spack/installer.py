...
def _install_from_cache(pkg, cache_only, explicit, unsigned=False, full_hash_match=False):
    """
...
def _process_binary_cache_tarball(pkg, binary_spec, explicit, unsigned, preferred_mirrors=None):
    """
...
def _try_install_from_binary_cache(pkg, explicit, unsigned=False, full_hash_match=False):
    ...
    matches = binary_distribution.get_mirrors_for_spec(pkg.spec, full_hash_match=full_hash_match)
...
class PackageInstaller(object):
    ...
    def _install_task(self, task):
        ...
        if use_cache and _install_from_cache(pkg, cache_only, explicit, unsigned, full_hash_match):
            self._update_installed(task)
            if task.compiler:
        ...
        try:
            ...
            spack.package.PackageBase._verbose = spack.build_environment.start_build_process(
