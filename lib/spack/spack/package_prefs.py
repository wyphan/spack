...
class PackagePrefs(object):
    ...
    def preferred_variants(cls, pkg_name):
        ...
        return dict(
            (name, variant) for name, variant in spec.variants.items() if name in pkg.variants
        )
...
def is_spec_buildable(spec):
    ...
    spec_reversed = spec.name in reverse or any(spec.package.provides(name) for name in reverse)
