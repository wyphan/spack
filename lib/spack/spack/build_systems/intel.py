...
class IntelPackage(PackageBase):
    ...
    def openmp_libs(self):
        ...
        if "%intel" in self.spec:
            ...
            omp_lib_path = gcc("--print-file-name", "libgomp.%s" % dso_suffix, output=str)
            omp_libs = LibraryList(omp_lib_path.strip())

        elif "%clang" in self.spec:
            clang = Executable(self.compiler.cc)
            omp_lib_path = clang("--print-file-name", "libomp.%s" % dso_suffix, output=str)
            omp_libs = LibraryList(omp_lib_path.strip())
    ...
    def _tbb_abi(self):
        ...
        matches = re.search(
            r"(gcc|LLVM).* ([0-9]+\.[0-9]+\.[0-9]+).*", gcc("--version", output=str), re.I | re.M
        )
        abi = ""
