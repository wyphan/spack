...
def test_get_concrete_specs(config, mutable_mock_env_path, mock_packages):
    ...
    with e as active_env:
        ...
        spec_map = ci.get_concrete_specs(active_env, dyninst_hash, "dyninst", dep_builds, "NONE")
        ...
        spec_map = ci.get_concrete_specs(active_env, s.name, s.name, dep_builds, "FIND_ANY")
...
def test_register_cdash_build(monkeypatch):
    ...
    build_id, build_stamp = ci.register_cdash_build(build_name, base_url, project, site, track)

    assert build_id == 42


def test_relate_cdash_builds(config, mutable_mock_env_path, mock_packages, monkeypatch, capfd):
    e = ev.create("test1")
    e.add("dyninst")
    e.concretize()
    ...
    with e as active_env:
        ...
        monkeypatch.setattr(ci, "read_cdashid_from_mirror", lambda s, u: dep_cdash_ids.pop(s.name))
