...
def test_ci_generate_for_pr_pipeline(
    tmpdir, mutable_mock_env_path, install_mockery, mock_packages, monkeypatch, project_dir_env
):
...
"""
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test"):
            ...
            monkeypatch.setattr(ci, "SPACK_SHARED_PR_MIRROR_URL", r"file:///fake/mirror_two")
            try:
                ci_cmd("generate", "--output-file", outputfile)
            finally:
...
""".format(
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test") as env:
            ...
            buildcache_cmd(
                "download", "--spec-file", json_path, "--path", dl_dir.strpath, "--require-cdashid"
            )
