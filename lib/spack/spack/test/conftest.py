...
def mock_git_version_info(tmpdir, scope="function"):
    ...
    with working_dir(repo_path):
        ...
        log = git("log", "--all", "--pretty=format:%H", "--topo-order", output=str, error=str)
...
def mock_pkg_install(monkeypatch):
    monkeypatch.setattr(spack.package.PackageBase, "install", _pkg_install_fn, raising=False)
...
def mock_configuration_scopes(configuration_dir):
    ...
    defaults = spack.config.InternalConfigScope("_builtin", spack.config.config_defaults)
