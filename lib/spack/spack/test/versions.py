...
def test_versions_from_git(mock_git_version_info, monkeypatch, mock_packages):
    ...
    monkeypatch.setattr(spack.package.PackageBase, "git", "file://%s" % repo_path, raising=False)

    for commit in commits:
        spec = spack.spec.Spec("git-test-commit@%s" % commit)
        version = spec.version
        comparator = [
            str(v) if not isinstance(v, int) else v for v in version._cmp(version.commit_lookup)
        ]
