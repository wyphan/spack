...
def test_pgi_link_paths():
    check_link_paths("pgcc-16.3.txt", ["/usr/tce/packages/pgi/pgi-16.3/linux86-64/16.3/lib"])
...
def test_xl_link_paths():
    check_link_paths(
        "xl-13.1.5.txt",
        ["/opt/ibm/xlsmp/4.1.5/lib", "/opt/ibm/xlmass/8.1.5/lib", "/opt/ibm/xlC/13.1.5/lib"],
    )
...
def test_obscure_parsing_rules():
    check_link_paths("obscure-parsing-rules.txt", ["/first/path", "/second/path", "/third/path"])
