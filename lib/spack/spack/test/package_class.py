...
def test_possible_dependencies(mock_packages, mpileaks_possible_deps):
    ...
    assert mpileaks_possible_deps == (mpileaks.possible_dependencies(expand_virtuals=True))
...
def test_possible_dependencies_virtual(mock_packages, mpi_names):
    expected = dict((name, set(spack.repo.get(name).dependencies)) for name in mpi_names)
