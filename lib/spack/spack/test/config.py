...
def test_substitute_config_variables(mock_low_high_config, monkeypatch):
    ...
    assert os.path.join("/foo/bar/baz", prefix, "foo/bar/baz") == spack_path.canonicalize_path(
        "/foo/bar/baz/$spack/foo/bar/baz/"
    )
...
def test_substitute_user(mock_low_high_config):
    ...
    assert "/foo/bar/" + user + "/baz" == spack_path.canonicalize_path("/foo/bar/$user/baz")


def test_substitute_user_cache(mock_low_high_config):
    user_cache_path = spack.paths.user_cache_path
    assert user_cache_path + "/baz" == spack_path.canonicalize_path("$user_cache_path/baz")


def test_substitute_tempdir(mock_low_high_config):
    tempdir = tempfile.gettempdir()
    assert tempdir == spack_path.canonicalize_path("$tempdir")
    assert tempdir + "/foo/bar/baz" == spack_path.canonicalize_path("$tempdir/foo/bar/baz")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [["config:install_tree:root", "/path"], ["config:install_tree:padded_length", 11]],
        ...
        (
            [["config:install_tree:padded_length", True], ["config:install_tree:root", "/path"]],
            [full_padded_string, "/path", None],
        ),
        ([["config:install_tree:", "/path$padding"]], [full_padded_string, "/path", None]),
        ([["config:install_tree:", "/path/${padding}"]], [full_padded_string, "/path", None]),
    ],
)
...
def test_dotkit_in_config_does_not_raise(mock_low_high_config, write_config_file, capsys):
    write_config_file("config", {"config": {"module_roots": {"dotkit": "/some/path"}}}, "high")
