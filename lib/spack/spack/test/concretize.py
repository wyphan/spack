...
class TestConcretize(object):
    ...
    def test_architecture_deep_inheritance(self, mock_targets):
        ...
        with spack.repo.use_repositories(mock_repo):
            spec = Spec(
                "foopkg %gcc@4.5.0 os=CNL target=nocona" + " ^barpkg os=CNL ^bazpkg os=CNL"
            )
            spec.concretize()
    ...
    def test_virtual_is_fully_expanded_for_mpileaks(self):
        ...
        assert (
            "fake" in spec._dependencies["callpath"].spec._dependencies["zmpi"].spec._dependencies
        )  # NOQA: ignore=E501
        assert all("mpi" not in d._dependencies for d in spec.traverse())
        assert "zmpi" in spec
        assert "mpi" in spec
    ...
    @pytest.mark.xfail()
    def test_reuse_installed_packages(self, context, mutable_database, repo_with_changing_recipe):
        # Install a spec
