...
class Environment(object):
    ...
    def remove(self, query_spec, list_name=user_speclist_name, force=False):
        ...
        if not matches:
            ...
            dag_hashes_in_order = [
                self.specs_by_hash[build_hash].dag_hash() for build_hash in self.concretized_order
            ]
        ...
        for user_spec, concretized_user_spec in self.concretized_specs():
            ...
            if spec.concrete:
                ...
                is_match = (
                    ...
                    and concretized_user_spec[spec.name].build_hash() == spec.build_hash()
                )
                if is_match:
                    matches[spec] = spec
                continue
    ...
    def _read_lockfile_dict(self, d):
        ...
        for build_hash, node_dict in json_specs_by_hash.items():
            for _, dep_hash, deptypes, _ in Spec.dependencies_from_node_dict(node_dict):
                specs_by_hash[build_hash]._add_dependency(specs_by_hash[dep_hash], deptypes)
        ...
        if old_hash_to_new:
            ...
            self.concretized_order = [old_hash_to_new.get(h, h) for h in self.concretized_order]
