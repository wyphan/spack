...
class _BuildcacheBootstrapper(object):
    ...
    def _read_metadata(self, package_name):
        ...
        json_path = os.path.join(spack.paths.share_path, "bootstrap", self.name, json_filename)
...
def ensure_module_importable_or_raise(module, abstract_spec=None):
    ...
    for current_config in source_configs:
        if not _source_is_trusted(current_config):
            msg = (
                '[BOOTSTRAP MODULE {0}] Skipping source "{1}" since it is ' "not trusted"
    ...
    for method in errors:
        err = errors[method]
        msg += "    '{0}' raised {1}: {2}\n".format(method, err.__class__.__name__, str(err))
...
def ensure_executables_in_path_or_raise(executables, abstract_spec):
    ...
    for current_config in source_configs:
        if not _source_is_trusted(current_config):
            msg = (
                '[BOOTSTRAP EXECUTABLES {0}] Skipping source "{1}" since it is ' "not trusted"
