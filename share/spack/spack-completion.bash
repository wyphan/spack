...
_spack_concretize() {
    SPACK_COMPREPLY="-h --help -f --force --test --fresh --reuse"
}
...
_spack_dev_build() {
    if $list_options
    then
        SPACK_COMPREPLY="-h --help -j --jobs -d --source-path -i --ignore-dependencies -n --no-checksum --deprecated --keep-prefix --skip-patch -q --quiet --drop-in --test -b --before -u --until --clean --dirty --fresh --reuse"
    else
        _all_packages
    fi
}
...
_spack_solve() {
    if $list_options
    then
        SPACK_COMPREPLY="-h --help --show --models -l --long -L --very-long -I --install-status -y --yaml -j --json -c --cover -N --namespaces -t --types --timers --stats --fresh --reuse"
    else
        _all_packages
    fi
}
...
_spack_spec() {
    if $list_options
    then
        SPACK_COMPREPLY="-h --help -l --long -L --very-long -I --install-status -y --yaml -j --json -c --cover -N --namespaces --hash-type -t --types --fresh --reuse"
    else
        _all_packages
    fi
}
