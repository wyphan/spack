...
class Sundials(CMakePackage, CudaPackage, ROCmPackage):
    ...
    # Monitoring
    variant(
        "monitoring", default=False, description="Build with simulation monitoring capabilities"
    )
    ...
    def cmake_args(self):
        ...
        if (spec.satisfies("%apple-clang")) and ("+fcmix" in spec):
            ...
            libgfortran = LibraryList(f77("--print-file-name", "libgfortran.a", output=str))
        ...
        # Monitoring
        args.extend([self.define_from_variant("SUNDIALS_BUILD_WITH_MONITORING", "monitoring")])

        # Profiling
        args.extend([self.define_from_variant("SUNDIALS_BUILD_WITH_PROFILING", "profiling")])
        if "+profiling+caliper" in spec:
            args.extend(["-DENABLE_CALIPER=ON", "-DCALIPER_DIR=%s" % spec["caliper"].prefix])
        ...
        if "+superlu-mt" in spec:
            if spec.satisfies("@3.0.0:"):
                args.extend(["-DBLAS_ENABLE=ON", "-DBLAS_LIBRARIES=%s" % spec["blas"].libs])
            args.extend(
                [
        ...
        if "+trilinos" in spec:
            args.extend(["-DTrilinos_ENABLE=ON", "-DTrilinos_DIR=%s" % spec["trilinos"].prefix])
        else:
