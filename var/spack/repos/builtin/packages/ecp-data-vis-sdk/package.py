...
class EcpDataVisSdk(BundlePackage, CudaPackage):
    ...
    dav_sdk_depends_on("ascent+shared+mpi+fortran+openmp+python+vtkh+dray", when="+ascent")
    ...
    dav_sdk_depends_on(
        "vtk-m+shared+mpi+openmp+rendering", when="+vtkm", propagate=["cuda"] + cuda_arch_variants
    )
