...
class Flake8(Package):
    ...
    variant(
        "somewhat-awesome-feature", default=False, description="Enable somewhat awesome feature"
    )

    provides("somevirt", when="@2.0+super-awesome-feature+somewhat-awesome-feature")

    extends(
        "python", ignore="bin/(why|does|every|package|that|depends|on|numpy|need|to|copy|f2py3?)"
    )
